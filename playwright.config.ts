...
export default defineConfig({
  ...
  webServer: {
    ...
    // page.route) sans dépendre d'un vrai déploiement Cloudflare. En https
    // (pas http) : la CSP de la page (voir index.html) restreint connect-src
    // à 'self' et https:.
    command:
      'VITE_SYNC_WORKER_URL=https://sync.invalid npm run build && npm run preview -- --host 127.0.0.1 --port 4173',
